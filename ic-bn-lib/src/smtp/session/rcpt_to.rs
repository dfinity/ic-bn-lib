...
use crate::{
    ...
    smtp::{RecipientPolicy, RecipientResolveError, address::EmailAddress, session::Session},
};

impl<S: AsyncReadWrite> Session<S> {
    /// Handles RCPT TO command
    pub async fn handle_rcpt_to(&mut self, to: RcptTo<Cow<'_, str>>) -> io::Result<()> {
        ...
        match self.recipient_resolver.resolve_recipient(&address).await {
            Ok(v) => match v {
