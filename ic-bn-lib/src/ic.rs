...
pub trait AgentExt {
    ...
    /// Fetches all subnet IDs & their data concurrently. Shortcut for `fetch_subnet_ids` + `fetch_subnet_data`.
    /// Returns a map from a subnet ID to [`SubnetData`] struct (or an error) that contains the subnet type and its canister ranges.
    /// `concurrency` defaults to 1 if unspecified.
...
impl AgentExt for Agent {
    ...
    async fn fetch_all_subnets_data(
        ...
        let subnet_ids = self
            .fetch_subnet_ids(root_subnet_id)
            .await
            .map_err(|e| AgentError::MessageError(format!("Failed to fetch subnet IDs: {e:#}")))?;

        let futures = subnet_ids
            .into_iter()
            .map(|x| async move { (x, self.fetch_subnet_data(&x).await) });

        let stream = futures::stream::iter(futures).buffer_unordered(concurrency.unwrap_or(1));
        let results = AHashMap::from_iter(stream.collect::<Vec<_>>().await);
...
mod test {
    ...
    #[tokio::test]
    async fn fetch_all_subnets_data_verifies_against_fake_root_key() {
        let root_subnet_id = Principal::from_slice(&[0xaa; 10]);
        let subnet_a = Principal::from_slice(&[0x01; 10]);
        let subnet_b = Principal::from_slice(&[0x02; 10]);

        let range_a = (
            Principal::from_slice(&[0x00; 10]),
            Principal::from_slice(&[0x7f; 10]),
        );
        let range_b = (
            Principal::from_slice(&[0x80; 10]),
            Principal::from_slice(&[0xff; 10]),
        );
        ...
        let subnet_inner = |subnet_type: &str| {
            tree_of(vec![
                (b"node".to_vec(), empty()),
                (b"public_key".to_vec(), leaf(vec![9, 9, 9])),
                (b"type".to_vec(), leaf(subnet_type.as_bytes().to_vec())),
            ])
        };

        let ranges_tree = |range: (Principal, Principal)| {
            let ranges_cbor = serde_cbor::to_vec(&vec![range]).unwrap();
            tree_of(vec![(b"shard".to_vec(), leaf(ranges_cbor))])
        };
        ...
        let tree = tree_of(vec![
            (
                b"canister_ranges".to_vec(),
                tree_of(vec![
                    (subnet_a.as_slice().to_vec(), ranges_tree(range_a)),
                    (subnet_b.as_slice().to_vec(), ranges_tree(range_b)),
                ]),
            ),
            (
                b"subnet".to_vec(),
                tree_of(vec![
                    (subnet_a.as_slice().to_vec(), subnet_inner("application")),
                    (subnet_b.as_slice().to_vec(), subnet_inner("system")),
                ]),
            ),
            (b"time".to_vec(), leaf(leb128(now_ns))),
        ]);
        ...
        // Non-default concurrency to also exercise the buffer_unordered() path.
        let results = agent
            .fetch_all_subnets_data(root_subnet_id, Some(4))
            .await
            .unwrap();

        assert_eq!(results.len(), 2);
        assert_eq!(
            results.get(&subnet_a).unwrap().as_ref().unwrap(),
            &SubnetData {
        ...
        assert_eq!(
            results.get(&subnet_b).unwrap().as_ref().unwrap(),
            &SubnetData {
