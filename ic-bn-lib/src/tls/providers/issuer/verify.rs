...
#[cfg_attr(test, automock)]
pub(super) trait Verify: Sync + Send {
...
impl<P: Parse> Verify for Verifier<P> {
    fn verify(&self, pkg: &Package) -> Result<(), VerifyError> {
        ...
        {
            return Err(VerifyError::InvalidDomainName(pkg.name.clone()));
        }
        ...
        if pkg.name != cn {
            return Err(VerifyError::CommonNameMismatch(pkg.name.clone(), cn));
        }
...
mod tests {
    ...
    use super::{super::Pair, MockParse, Package, Verifier, Verify, VerifyError};
